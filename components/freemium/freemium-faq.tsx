  ...
  return (
    <section className="rounded-3xl border border-white/50 bg-white/75 p-8 shadow-xl shadow-slate-900/5 backdrop-blur-xl">
      ...
      <div className="mt-8 grid gap-3">
        {freemiumFaq.map((item) => (
          <details
            key={item.question}
            className="group rounded-3xl border border-white/50 bg-white/80 p-1 shadow-lg shadow-slate-900/5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
          >
            <summary className="flex cursor-pointer list-none items-center justify-between gap-5 rounded-3xl px-5 py-4">
              <span className="text-base font-semibold text-ink">
                {item.question}
              </span>
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-action via-sky-500 to-violet-500 text-lg font-semibold text-white transition-all duration-300 group-open:rotate-45">
                +
              </span>
            </summary>
