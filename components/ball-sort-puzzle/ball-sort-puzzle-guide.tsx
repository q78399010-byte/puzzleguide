...
const ballSortSlug = "ball-sort-puzzle";

const guideCards = [
  {
    title: "Guide",
    text: "Understand tube pressure, anchor colors, and when to delay mixed stacks.",
    href: routes.game(ballSortSlug)
  },
  {
    title: "Walkthrough",
    text: "Jump into written level paths for empty tube timing, color stacks, and cleanup order.",
    href: routes.walkthrough(ballSortSlug)
  },
  {
    title: "Solutions",
    text: "Use concise solution notes when a board locks near the final ball groups.",
    href: routes.solution(ballSortSlug)
  }
];

export function BallSortPuzzleGuide() {
  return (
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        <div>
          ...
          <h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight text-ink sm:text-6xl">
            Ball Sort Guides & Solutions
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted sm:text-xl">
            Walkthroughs, tube strategies, color order tips and puzzle solutions.
          </p>
        </div>
        <div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-2xl shadow-blue-500/10 backdrop-blur-xl">
          ...
          <p className="mt-3 text-2xl font-bold tracking-tight text-ink">
            Keep one empty tube before moving rare colors.
          </p>
          <p className="mt-3 text-sm leading-6 text-muted">
            Ball Sort levels usually fail when every spare tube is filled too
            early. Build one clean color stack first, then use empty tubes to
            unlock the final groups.
          </p>
        </div>
      </div>
