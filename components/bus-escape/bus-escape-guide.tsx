...
const guideCards = [
  {
    title: "Guide",
    text: "Understand bus lanes, blocked exits, and when to delay short vehicle moves.",
    href: routes.game("bus-escape")
  },
  {
    title: "Walkthrough",
    text: "Jump into written level paths for lane order, bus timing, and cleanup moves.",
    href: routes.walkthrough("bus-escape")
  },
  {
    title: "Solutions",
    text: "Use concise solution notes when a board locks near the final bus exit.",
    href: routes.solution("bus-escape")
  }
];

export function BusEscapeGuide() {
  return (
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        <div>
          ...
          <h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight text-ink sm:text-6xl">
            Bus Escape Guides & Solutions
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted sm:text-xl">
            Walkthroughs, bus lane tips, exit strategies and puzzle solutions.
          </p>
        </div>
        <div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-2xl shadow-blue-500/10 backdrop-blur-xl">
          ...
          <p className="mt-3 text-2xl font-bold tracking-tight text-ink">
            Clear the main bus lane before moving side blockers.
          </p>
          <p className="mt-3 text-sm leading-6 text-muted">
            Bus Escape levels usually fail when the exit lane closes early.
            Open the main route first, then move short vehicles only when they
            create room for the next bus.
          </p>
        </div>
      </div>
