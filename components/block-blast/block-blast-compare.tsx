const games = ["Block Blast", "Hexa Sort", "Goods Sort", "Water Sort"];

const rows = [
  {
    label: "Difficulty",
    values: ["Medium", "Medium", "Medium", "Easy"]
  },
  {
    label: "Relaxation",
    values: ["Medium", "High", "High", "High"]
  },
  {
    label: "Strategy",
    values: ["Board control", "Stack merging", "Shelf grouping", "Color storage"]
  },
  {
    label: "Learning Curve",
    values: ["Moderate", "Moderate", "Easy", "Easy"]
  },
  {
...
export function BlockBlastCompare() {
  return (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-4xl font-bold tracking-tight text-ink">
            Block Blast vs Hexa Sort vs Goods Sort vs Water Sort
          </h2>
        </div>
        <p className="max-w-md text-sm leading-6 text-muted">
          Use this quick comparison to choose between board control, stack
          merging, shelf sorting, and color storage puzzle sessions.
        </p>
      </div>
