...
const solverStats = [
  {
    label: "Best Move",
    value: "Release R1 first",
    text: "Open the tray lane before matching the back screws."
  },
  {
    label: "Combo Chance",
    value: "68%",
    text: "Two color trays can chain if the front blockers move now."
  },
  {
    label: "Danger Level",
    value: "Medium",
    text: "One slot stays open, but the lower row is getting tight."
  },
  {
    label: "Next Strategy",
    value: "Hold one empty tray",
    text: "Keep space for the next screw color before clearing P5."
  }
];

export function ScrewJamSolverPreview() {
  return (
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-action">
            Screw Jam Solver Preview
          </p>
          <h3 className="mt-3 text-2xl font-bold tracking-tight text-ink">
            Tray-first screw route
          </h3>
          <p className="mt-3 text-sm leading-6 text-muted">
            Static mock data for a compact screw board.
          </p>
        </div>
