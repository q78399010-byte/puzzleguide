import type { ConfidenceMapItem } from "@/components/intelligence-center/intelligence-center-data";

type ConfidenceMapCardProps = {
  items: ConfidenceMapItem[];
};

export function ConfidenceMapCard({ items }: ConfidenceMapCardProps) {
  return (
    <section className="rounded-3xl border border-white/60 bg-white/75 p-5 shadow-xl shadow-slate-900/5 backdrop-blur-xl sm:p-6">
      ...
      <h3 className="mt-3 text-3xl font-bold tracking-tight text-ink">
        Static solver confidence levels.
      </h3>
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm font-bold text-ink">{item.game}</span>
