...
const guideCards = [
  {
    title: "Guide",
    text: "Understand anchor colors, locked containers, and when to delay mixed moves.",
    href: routes.game("magic-sort")
  },
  {
    title: "Walkthrough",
    text: "Jump into written level paths for storage timing, color priority, and cleanup order.",
    href: routes.walkthrough("magic-sort")
  },
  {
    title: "Solutions",
    text: "Use concise solution notes when a board locks near the final color groups.",
    href: routes.solution("magic-sort")
  }
];

export function MagicSortGuide() {
  return (
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        <div>
          ...
          <h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight text-ink sm:text-6xl">
            Magic Sort Guides & Solutions
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted sm:text-xl">
            Walkthroughs, color sorting tips, storage strategies and puzzle solutions.
          </p>
        </div>
        <div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-2xl shadow-blue-500/10 backdrop-blur-xl">
          ...
          <p className="mt-3 text-2xl font-bold tracking-tight text-ink">
            Build one clean anchor color before spending temporary storage.
          </p>
          <p className="mt-3 text-sm leading-6 text-muted">
            Magic Sort levels usually fail when every spare container fills too
            early. Keep one working space open until the next color group can
            merge cleanly.
          </p>
        </div>
      </div>
