...
const guideCards = [
  {
    title: "Guide",
    text: "Understand hex tile stacks, board pressure, and when to delay merges.",
    href: routes.game("hexa-sort")
  },
  {
    title: "Walkthrough",
    text: "Jump into written level paths for tile placement, stack timing, and cleanup order.",
    href: routes.walkthrough("hexa-sort")
  },
  {
    title: "Solutions",
    text: "Use concise solution notes when a board locks near the final hex groups.",
    href: routes.solution("hexa-sort")
  }
];

export function HexaSortGuide() {
  return (
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
        <div>
          ...
          <h2 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight text-ink sm:text-6xl">
            Hexa Sort Guides & Solutions
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-muted sm:text-xl">
            Walkthroughs, hex tile stacking, merge strategies and puzzle solutions.
          </p>
        </div>
        <div className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-2xl shadow-blue-500/10 backdrop-blur-xl">
          ...
          <p className="mt-3 text-2xl font-bold tracking-tight text-ink">
            Build one clean stack before filling the board.
          </p>
          <p className="mt-3 text-sm leading-6 text-muted">
            Hexa Sort levels usually fail when every open cell is filled with
            mixed colors. Anchor one stack first, then use the remaining board
            space to prepare controlled merges.
          </p>
        </div>
      </div>
