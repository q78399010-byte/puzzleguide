const games = ["Hexa Sort", "Goods Sort", "Water Sort", "Block Blast"];

const rows = [
  {
    label: "Difficulty",
    values: ["Medium", "Medium", "Easy", "Medium"]
  },
  {
    label: "Relaxation",
    values: ["High", "High", "High", "Medium"]
  },
  {
    label: "Strategy",
    values: ["Stack merging", "Shelf grouping", "Color storage", "Board control"]
  },
  {
    label: "Learning Curve",
    values: ["Moderate", "Easy", "Easy", "Moderate"]
  },
  {
    label: "Average Session",
    values: ["3 min", "3 min", "3 min", "2 min"]
  }
];

export function HexaSortCompare() {
  return (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-4xl font-bold tracking-tight text-ink">
            Hexa Sort vs Goods Sort vs Water Sort vs Block Blast
          </h2>
        </div>
        <p className="max-w-md text-sm leading-6 text-muted">
          Use this quick comparison to choose between hex stack merging, shelf
          sorting, bottle storage, and board-control puzzle sessions.
        </p>
      </div>
